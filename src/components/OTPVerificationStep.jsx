...
const OTPVerificationStep = ({ userInfo, handleback, onVerifyOTP,setAlertMessage, setShowAlert,countryCode }) => {
  const [otp, setOtp] = useState('');
const [timeLeft, setTimeLeft] = useState(() => {
  const lastSent = parseInt(localStorage.getItem('otpRequestTime') || '0');
  const now = Date.now();
...
const handleResend = () => {
  ...
  const now = Date.now();
  localStorage.setItem('otpRequestTime', now.toString());
  setTimeLeft(300);
  setOtp('');
    ...
    <div style={styles.container}>
      ...
      <form style={styles.form} onSubmit={handleSubmit}>
        <label style={styles.label}>
          Enter OTP sent to +91 {userInfo?.phone}
        </label>
        <input
          type="text"
          maxLength="6"
