...
import { Video, VideoOff, Mic, MicOff, PhoneOff, Phone, X, Settings } from 'lucide-react';
...
const startJSRingtone = () => {
  try {
    stopJSRingtone(); // safety
    ...
    // stop after 40 sec max
    ringtoneTimer.current = setTimeout(() => {
      stopJSRingtone();
    }, 40000);

  } catch (e) {
    console.error("start ringtone err", e);
  }
};

const stopJSRingtone = () => {
  try {
    if (ringtoneTimer.current) {
      clearTimeout(ringtoneTimer.current);
