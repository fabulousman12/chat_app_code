...
import { FaPaperclip, FaImage } from "react-icons/fa"; // Font Awesome icons
...
const Chatwindo = ({ db,socket,setMessages,saveMessage,selectedUser, messagesRef,blockUser,unblockUser,blockedUsers,setMessagestest,message,storeMessageInSQLite,setmutedList,setUsersMain,host,customSounds, setCustomSounds }) => {
...
localStorage.setItem('usersMain', JSON.stringify(updatedUsers));
    ...
    useEffect(() => {
  const updateChatUI = () => {
    // Avoid duplicates by tracking seen message IDs
    const seenIds = new Set();

    // Filter only messages relevant to the current chat user
    const filteredMessages = messagesRef.current
      .filter(msg =>
        msg.sender === userdetails.id || msg.recipient === userdetails.id
      )
      .filter(msg => {
        if (seenIds.has(msg.id)) return false;
        seenIds.add(msg.id);
...
const toggleMute = () => {
  ...
  if (mutedUsers && mutedUsers.includes(userdetails.id)) {
    ...
    localStorage.setItem('mutedUsers', JSON.stringify(mutedUsers));
         Preferences.set({
      key: 'mutedUsers',
      value: JSON.stringify(mutedUsers), 
    });
    setmutedList(prev => prev.filter(id => id !== userdetails.id));
    setIsMuted(true);
  }
...
const handleMediaSelect = (e) => {
  ...
  const cleanedFiles = files.map(file => ({
    ...
    preview: file.preview,       // keep reference to native File if needed for upload
  }));
...
const handleFileProcess = (file, token) => {
  ...
  //    setUploadingFiles(prev => ({ ...prev, [file.name]: true }));
      ...
      return waitForResult(uploadFile(file, token), 'uploading file')
        .then(signedUrl => {
          if (!signedUrl?.fileUrl) throw new Error("Upload returned invalid URL");
          console.log("✅ Upload complete:", signedUrl.fileUrl);
...
onClick={() => isExpanded ? toglebigscreen() : toggleHeader()}
                />
                <div
                  className="absolute bottom-3 right-3 w-5 h-5 bg-green-500 rounded-full border-4 shadow-md"
...
>
 ...
 backgroundImage: " linear-gradient(rgba(238, 237, 239, 0.9), rgba(238, 237, 239, 0.9)), url('/circuit-board/circuit-board.svg')",
...
{msg.isDownload === 1  ? (
  ...
  className="p-3 flex items-center gap-3 bg-white/80 backdrop-blur-md border-t border-gray-200 shadow-lg"
  style={{
    zIndex: 100,
    minHeight: "100px",
    borderRadius: "12px",
...
>
  ...
  placeholder={  isTargetBlocked ? 'You blocked this user' : 'Type a message'}
          style={{borderRadius : '20px',color:'#ffffff',width: '30%'}}
        />
  ...
  >
    ...
    <div className="flex-1 flex items-center justify-center w-full">
      {mediaFiles[activeMediaIndex]?.type?.startsWith("image/") ? (
        <img
          src={mediaFiles[activeMediaIndex].preview} // ✅ base64 preview
          alt=""
      ...
      {mediaFiles.map((file, index) => (
        ...
        >
          ...
          {file.type.startsWith("image/") ? (
            <img
              src={file.preview} // ✅ base64 image
              alt=""
              className="w-full h-full object-cover"
            />
          ) : (
            <video
              src={file.preview} // ✅ base64 video
              className="w-full h-full object-cover"
            />
          )}
        </div>
